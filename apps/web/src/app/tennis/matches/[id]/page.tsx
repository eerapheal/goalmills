                ...
                <div className="glass-card rounded-xl p-6 mb-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white/5 rounded-lg p-4 text-center">
                            ...
                            <p className="text-2xl font-bold text-yellow-500">{odds['Match Winner']['Home']?.['Bet365'] || '-'}</p>
                        </div>
                        <div className="bg-white/5 rounded-lg p-4 text-center">
                            <p className="text-sm text-text-muted mb-2">{match.event_second_player}</p>
                            <p className="text-2xl font-bold text-yellow-500">{odds['Match Winner']['Away']?.['Bet365'] || '-'}</p>
                        </div>
                    </div>
                </div>
            );
        ...
        <div className="min-h-screen bg-[#0a0e27] pt-[90px] pb-10">
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="flex items-center gap-4 mb-6">
                    ...
                    <span className="text-sm font-bold text-text-muted uppercase tracking-wider">Match Details</span>
                </div>

                {/* Match Header Card */}
                <div className="glass-card rounded-2xl p-6 mb-6 border border-white/5">
                    <div className="text-center mb-4">
                        <Link href={`/tennis/leagues/${match.league_key}`} className="text-sm font-bold text-text-secondary uppercase tracking-wider hover:text-yellow-500 transition-colors">
                            {match.league_name} - {match.league_round}
                        </Link>
                        <p className={`text-xs font-bold mt-2 uppercase tracking-wider ${match.event_live === '1' ? 'text-red-500' : 'text-blue-400'}`}>
                    ...
                    <div className="flex items-center justify-between gap-8">
                        ...
                        <div className="text-center">
                            ...
                            {match.event_live === '1' && <p className="text-yellow-500 font-bold">{match.event_game_result}</p>}
                        </div>
