    ...
    if (!player) {
        return (
            <div className="min-h-screen bg-[#0a0e27] pt-[90px] flex flex-col justify-center items-center text-white">
                ...
                <button onClick={() => router.back()} className="text-yellow-500 hover:underline">Go Back</button>
            </div>
        );
    }
            ...
            <div className="max-w-4xl mx-auto px-4">
                ...
                {currentStats && (
                    <div className="glass-card rounded-2xl p-6 mb-8 border border-white/5">
                        <h2 className="text-2xl font-bold text-white mb-6 pl-2 border-l-4 border-yellow-500">{currentStats.season} Stats</h2>
                        ...
                        <div className="grid grid-cols-3 gap-4 mb-6">
                            ...
                            <div className="bg-white/5 rounded-lg p-4 text-center">
                                <p className="text-3xl font-black text-yellow-500 mb-1">{currentStats.matches_won}/{currentStats.matches_lost}</p>
                        ...
                        <div className="border-t border-white/5 pt-4">
                            <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-3">Surface Performance</h3>
                            <div className="grid grid-cols-3 gap-4">
                                <div className="bg-white/5 rounded-lg p-3 text-center">
                                    <p className="text-xs text-text-muted mb-1">Hard</p>
                                    <p className="text-lg font-bold text-white">{currentStats.hard_won}-{currentStats.hard_lost}</p>
                                </div>
                                <div className="bg-white/5 rounded-lg p-3 text-center">
                                    <p className="text-xs text-text-muted mb-1">Clay</p>
                                    <p className="text-lg font-bold text-white">{currentStats.clay_won}-{currentStats.clay_lost}</p>
                                </div>
                                <div className="bg-white/5 rounded-lg p-3 text-center">
                                    <p className="text-xs text-text-muted mb-1">Grass</p>
                                    <p className="text-lg font-bold text-white">{currentStats.grass_won}-{currentStats.grass_lost}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div>
                    <h2 className="text-2xl font-bold text-white mb-4 pl-2 border-l-4 border-yellow-500">🎾 Recent & Upcoming Matches</h2>
                    {matches.length > 0 ? (
                        matches.map(match => (
