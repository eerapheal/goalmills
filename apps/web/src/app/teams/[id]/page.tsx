...
import { FootballTeam, FootballLeague, FootballPlayer, FootballEvent, FootballStanding } from '@goalmills/types';
...
export default function TeamDetailsPage() {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                if (fixturesRes.result) {
                    ...
                    const finished = sortedMatches.filter(m =>
                        m.event_status === 'Finished' ||
                        new Date(`${m.event_date} ${m.event_time}`) < now
                    ...
                    if (finished.length > 0 || upcoming.length > 0) {
                        ...
                        if (leagueId) {
                            ...
                            if (standingsRes.result && standingsRes.result.total) {
                                const teamStanding = standingsRes.result.total.find(s => s.team_key === String(teamId));
    ...
    const getFormBadge = (match: FootballEvent) => {
        ...
        return (
            <div key={match.event_key} className={`w-8 h-8 rounded-full ${colors[result]} flex items-center justify-center font-bold text-xs border border-white/10`}>
                {result}
            </div>
        );
    };
                ...
                <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center gap-8 relative z-10">
                    ...
                    {recentMatches.length > 0 && (
                        <div className="flex flex-col items-center md:items-end gap-2">
                            <span className="text-sm font-bold text-text-muted uppercase tracking-wider">Recent Form</span>
                            <div className="flex gap-2">
                                {recentMatches.slice(0, 5).reverse().map(match => getFormBadge(match))}
                            </div>
                        </div>
                    )}
                </div>
                        ...
                        <div className="lg:col-span-1 bg-surface rounded-xl p-6 border border-white/5 h-full">
                            ...
                            <div className="flex items-center justify-around h-full pb-6">
                                <div className="text-center">
                                    <span className="block text-4xl font-black text-secondary mb-1">#{standing.standing_place}</span>
                                    <span className="text-sm text-text-muted">Rank</span>
                                </div>
                                <div className="w-px h-12 bg-white/10"></div>
                                <div className="text-center">
                                    <span className="block text-4xl font-black text-white mb-1">{standing.standing_PTS}</span>
                                    <span className="text-sm text-text-muted">Points</span>
                                </div>
                                <div className="w-px h-12 bg-white/10"></div>
                                <div className="text-center">
                                    <span className="block text-4xl font-black text-white mb-1">{standing.standing_P}</span>
                                ...
                                <Link href={`/matches/${match.event_key}`} key={`recent-${match.event_key}-${index}`} className="bg-surface p-4 rounded-xl border border-white/5 hover:border-white/20 transition-colors flex items-center justify-between group">
                                    ...
                                    <div className="flex-1 flex items-center justify-between px-4">
                                        <div className="flex items-center gap-2 flex-1 justify-end">
                                            <span className={`font-bold text-sm text-right ${match.home_team_key === String(teamId) ? 'text-white' : 'text-text-muted'}`}>{match.event_home_team}</span>
                                        ...
                                        <div className="flex items-center gap-2 flex-1 justify-start">
                                            ...
                                            <span className={`font-bold text-sm ${match.away_team_key === String(teamId) ? 'text-white' : 'text-text-muted'}`}>{match.event_away_team}</span>
                                        </div>
                                    </div>
                                </Link>
                            ))}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {upcomingMatches.map((match, index) => (
                                <Link href={`/matches/${match.event_key}`} key={`upcoming-${match.event_key}-${index}`} className="bg-surface p-5 rounded-xl border border-white/5 hover:border-secondary/50 transition-all group relative overflow-hidden">
                                    ...
                                    <div className="text-xs font-bold text-secondary mb-3 uppercase tracking-wider">{new Date(match.event_date).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })} • {match.event_time}</div>
