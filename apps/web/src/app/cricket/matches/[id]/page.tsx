      ...
      <div className="min-h-screen bg-[#0a0e27] pt-[120px] flex flex-col justify-center items-center px-4">
        <div className="glass-card max-w-md w-full p-8 rounded-3xl text-center border border-white/5">
          ...
          <p className="text-text-muted text-xs mb-6">
            The requested fixture could not be located in our real-time database feed.
          </p>
          <button
            onClick={() => router.back()}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div>
                ...
                Object.entries(match.comments).map(([innings, comments]) => (
                  <div key={innings} className="space-y-3">
                    ...
                    {comments.map((comment, i) => (
                      <div
                        key={i}
                        className="glass-card rounded-2xl p-4 border border-white/5 flex gap-4 hover:border-white/10 transition-all"
                      >
                  ...
                  <div className="glass-card rounded-2xl p-6 border border-white/5">
                    ...
                    <div className="space-y-2">
                      {match.lineups.home_team.starting_lineups.map((p, i) => (
                        ...
                        >
                          <span className="font-bold text-white hover:text-secondary uppercase transition-colors">
                            {p.player}
                          </span>
                          <span className="text-[10px] font-medium text-text-muted">
                            {p.player_country || 'Squad'}
                          </span>
                        </Link>
                      ))}
                    </div>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <span className="text-[10px] text-text-muted block">
                                Bowled by {wicket.balwer}
                              </span>
                            </div>
                          </div>

                          <div className="text-right">
                            <span className="font-bold text-amber-400 block tabular-nums">
                              {wicket.score}
                            </span>
                            <span className="text-[10px] text-text-muted block">
                              Over {wicket.fall}
          ...
          {activeTab === 'h2h' && (
            <div className="space-y-6">
              {(() => {
                const h2hList = Array.isArray(h2h) ? h2h : Array.isArray(h2h?.H2H) ? h2h.H2H : [];
                  ...
                  <div className="glass-card rounded-2xl p-6 border border-white/5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {h2hList.slice(0, 6).map((res, i) => (
                        <div
                          key={i}
                          className="p-4 rounded-xl bg-white/[0.02] border border-white/5 text-xs"
                        >
                ...
                <div className="glass-card rounded-2xl p-6 border border-white/5">
                  ...
                  <div className="space-y-4">
                    {Object.entries(odds).map(([market, outcomes]) => (
                      ...
                      >
                        <h4 className="text-xs font-bold text-amber-400 uppercase mb-3">
                          {market}
                        </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
              {[
                ...
                <div key={idx} className="glass-card rounded-2xl p-4 border border-white/5">
                  ...
                  <span className="text-xs font-bold text-white uppercase block">{item.value}</span>
                </div>
              ))}
            </div>
          )}
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center glass-card rounded-2xl border border-white/5 p-6">
      ...
      <p className="text-text-muted font-bold text-xs uppercase tracking-wider max-w-xs">
        {message}
      </p>
    </div>
  );
