  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const [matchesRes, standingsRes, squadsRes, venuesRes, newsRes, statsRes] =
          await Promise.all([
    ...
    return (
      <div className="min-h-screen bg-[#0a0e27] pt-[120px] flex flex-col justify-center items-center">
        ...
        <p className="text-text-secondary font-black uppercase tracking-widest text-xs">
          Syncing Tournament Intelligence...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#0a0e27] pt-[120px] flex flex-col justify-center items-center text-white px-4 text-center">
        ...
        <p className="text-text-secondary mb-8 max-w-md">
          The requested series data could not be retrieved from the worldwide feed.
        </p>
        <button
        ...
        <div className="glass-card rounded-3xl p-6 md:p-8 mb-8 border border-white/10 relative overflow-hidden bg-gradient-to-br from-white/[0.07] via-[#0a0e27] to-[#0d143d]">
          ...
          <div className="flex flex-col md:flex-row items-center gap-6 relative z-10">
            <div className="relative w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden bg-white/5 border border-white/10 flex-shrink-0 shadow-lg p-3 flex items-center justify-center">
              {series.league_logo ? (
                <img
                  src={series.league_logo}
                  alt={series.league_name}
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                <span className="text-[10px] font-bold text-white uppercase tracking-wider bg-white/5 px-3 py-1 rounded-lg border border-white/10">
                  {series.league_season || '2026'}
                </span>
                <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider bg-white/5 px-3 py-1 rounded-lg border border-white/10">
                      ...
                      {standings.map((row, idx) => (
                        <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                          <td className="py-3 px-4 flex items-center gap-3">
                            <span className="font-bold text-text-muted text-[11px] w-4">
                            ...
                            {row.team_logo && (
                              <img
                                src={row.team_logo}
                                alt={row.team_name}
                                className="w-6 h-6 object-contain rounded-md bg-white/5 p-0.5"
                              />
                            )}
                            <Link
                              href={`/cricket/teams/${row.team_key}`}
                              className="font-black text-white hover:text-secondary uppercase"
                            >
                              {row.team_name}
                            </Link>
                          </td>
                          ...
                          <td className="py-3 px-4 text-right font-bold text-text-muted tabular-nums">
                            {row.standing_NRR}
                          </td>
                        </tr>
                      ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {squads.length > 0 ? (
              squads.map((sq, idx) => (
                <div
                  key={idx}
                  className="glass-card rounded-2xl p-5 border border-white/5 hover:border-secondary/30 transition-all flex items-center justify-between"
                >
            ...
            {news.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {news.map((item, idx) => (
                  <div
                    key={idx}
                    className="glass-card rounded-2xl p-5 border border-white/5 hover:border-secondary/30 transition-all"
                  >
                    <span className="text-[10px] font-bold text-secondary uppercase tracking-wider block mb-2">
                      Tournament News
                    </span>
                    <h4 className="font-bold text-white text-sm mb-2">
                      {item.headline || item.title || 'Cricket News Update'}
                    </h4>
            ...
            {stats && stats.headers?.length ? (
              <div className="glass-card rounded-2xl p-6 border border-white/5">
                <h3 className="text-xs font-black text-white uppercase tracking-wider mb-4">
                  {stats.stat_type || 'Top Performers'}
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                      ...
                      {stats.values.map((row: any, rIdx: number) => (
                        <tr key={rIdx} className="hover:bg-white/[0.02]">
                          {row.values?.map((val: string, cIdx: number) => (
                            <td key={cIdx} className="py-2.5 px-3 font-semibold text-white">
                              {val}
                            </td>
                          ))}
                        </tr>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="glass-card rounded-3xl p-12 text-center border border-white/5 flex flex-col items-center justify-center">
      ...
      <p className="text-text-muted text-xs font-bold uppercase tracking-wider max-w-sm">
        {message}
      </p>
    </div>
  );
