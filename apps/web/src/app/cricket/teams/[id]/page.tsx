        ...
        <div className="min-h-screen bg-[#0a0e27] pt-[120px] pb-24">
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="flex items-center gap-6 mb-8">
                    ...
                    <div>
                        <span className="text-[10px] font-black text-secondary uppercase tracking-[0.3em]">Franchise / National Intel</span>
                        <h1 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter leading-none mt-1">{tName}</h1>
                    </div>
                </div>
                        ...
                        <div className="text-center md:text-left flex-1">
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-6">
                                <div className="p-3 rounded-2xl bg-white/5 border border-white/5 min-w-[100px] text-center">
                                    <span className="text-[9px] font-black text-text-muted uppercase tracking-widest block mb-0.5">Roster Count</span>
                                    <span className="text-base font-black text-white">{players.length} Athletes</span>
                                </div>
                                <div className="p-3 rounded-2xl bg-white/5 border border-white/5 min-w-[100px] text-center">
                                    <span className="text-[9px] font-black text-text-muted uppercase tracking-widest block mb-0.5">Upcoming</span>
                                    <span className="text-base font-black text-emerald-400">{schedules.length} Matches</span>
                                </div>
                                <div className="p-3 rounded-2xl bg-white/5 border border-white/5 min-w-[100px] text-center">
                                    <span className="text-[9px] font-black text-text-muted uppercase tracking-widest block mb-0.5">Completed</span>
                                    <span className="text-base font-black text-blue-400">{results.length} Matches</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex gap-4 mb-8 bg-white/5 p-2 rounded-2xl border border-white/5">
                    {[
                        ...
                        >
                            <span>{tab.icon}</span>
                            <span>{tab.label}</span>
                            <span className="px-1.5 py-0.5 rounded-md bg-black/30 text-[9px] font-black tabular-nums">{tab.count}</span>
                        </button>
                    ))}
                </div>
                        ...
                        <div>
                            {players.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {players.map((player) => (
                                        ...
                                        >
                                            ...
                                            <span className="text-secondary opacity-0 group-hover:opacity-100 transition-opacity text-sm font-black">→</span>
                                        </Link>
                                    ))}
                                </div>
                            ) : (
                                <div className="glass-card rounded-[2.5rem] p-16 text-center border-2 border-dashed border-white/5">
                                    ...
                                    <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2">Roster Synchronizing</h3>
                                    <p className="text-text-muted text-xs max-w-sm mx-auto">Official squad list for this tournament season is currently updating.</p>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        <div className="space-y-4">
                            {schedules.length > 0 ? (
                                ...
                                <div className="glass-card rounded-[2.5rem] p-16 text-center border-2 border-dashed border-white/5">
                                    ...
                                    <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2">No Scheduled Matches</h3>
                                    <p className="text-text-muted text-xs max-w-sm mx-auto">There are no upcoming tournament fixtures scheduled for this squad.</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {activeTab === 'results' && (
                        <div className="space-y-4">
                            {results.length > 0 ? (
                                ...
                                <div className="glass-card rounded-[2.5rem] p-16 text-center border-2 border-dashed border-white/5">
                                    <span className="text-4xl mb-4 block">✅</span>
                                    <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2">No Match Archive</h3>
                                    <p className="text-text-muted text-xs max-w-sm mx-auto">No recently completed fixtures recorded for this team in the archive.</p>
                                </div>
                            )}
                        </div>
                    )}
