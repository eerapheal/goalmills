...
async function requestApiFootball<T>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `[API-Football Web] Request to ${endpoint} failed: ${response.status} ${response.statusText}`
      );
    }
    ...
    if (
      ...
      console.warn(`[API-Football Web] Warnings/Errors on ${endpoint}:`, data.errors);
    }
