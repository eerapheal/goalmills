...
const generateMockMatches = (count: number, status: string, startId: number): CricketMatchInfo[] => {
  return Array.from({ length: count }).map((_, i) => {
    ...
    return {
      ...
      isFeatured: i % 3 === 0, // Every 3rd match is featured
      importance: 10 - i, // Descending importance
    };
  });
};
...
class CricketApi implements CricbuzzAPIClient {
  ...
  async getSeriesMatches(params: { seriesId: number }): Promise<CricketLiveMatchResponse> {
      await this.simulateDelay();
      const allMatches = [...mockLiveMatches, ...mockUpcomingMatches, ...mockRecentMatches];
      // In a real app, we filter by seriesId. Since all mock matches have seriesId 1, we just return them or a subset.
      const matches = allMatches.filter(m => m.seriesId === params.seriesId);
  ...
  async getFeaturedMatches(limit?: number): Promise<CricketLiveMatchResponse> {
      await this.simulateDelay();
      const allMatches = [...mockLiveMatches, ...mockUpcomingMatches, ...mockRecentMatches];
      const matches = allMatches.filter(m => m.isFeatured).slice(0, limit || 5);
      return { matches, totalMatches: matches.length };
  }
  ...
  async getImportantMatches(limit?: number): Promise<CricketLiveMatchResponse> {
      ...
      const matches = allMatches.sort((a, b) => (b.importance || 0) - (a.importance || 0)).slice(0, limit || 5);
      return { matches, totalMatches: matches.length };
  }
