...
export function TennisMatchCard({ match, onPress, odds }: TennisMatchCardProps) {
    ...
    const timeString = eventDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true });
        ...
        >
            ...
            <div className="flex items-center justify-between mb-3 pb-2 border-b border-white/5 relative z-10">
                <div onClick={(e) => e.stopPropagation()}>
                    <Link href={`/tennis/leagues/${match.league_key}`} className="text-[10px] font-bold text-text-secondary uppercase tracking-wider hover:text-white transition-colors">
                ...
                {odds && odds['Match Winner'] && (
                    <div className="flex justify-center gap-4 mb-2">
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded">
                            ...
                            <span className="text-xs font-bold text-yellow-500">{odds['Match Winner']['Home']?.['Bet365'] || '-'}</span>
                        </div>
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-white/5 rounded">
                            <span className="text-[10px] text-text-muted font-bold">2</span>
                            <span className="text-xs font-bold text-yellow-500">{odds['Match Winner']['Away']?.['Bet365'] || '-'}</span>
                        </div>
                    </div>
                )}

                <div className="flex justify-between items-center text-[10px] text-text-muted">
                    <span>{match.league_surface} • {match.country_name}</span>
