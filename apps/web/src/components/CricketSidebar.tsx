...
export function CricketSidebar() {
  ...
  return (
    <div className="space-y-6">
      ...
      {liveCount > 0 && (
        <div className="rounded-3xl border border-emerald-500/40 bg-gradient-to-br from-[#061C20] via-[#092E34] to-[#041215] p-4 shadow-xl shadow-emerald-950/40">
          <div className="flex items-center justify-between">
            ...
            <span className="px-2 py-0.5 rounded-full bg-emerald-500 text-slate-950 font-mono text-[10px] font-black">
              {liveCount} In Play
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {tournaments.map((tourn) => (
            ...
            >
              ...
              <FiArrowRight
                size={14}
                className="text-slate-500 group-hover:text-emerald-400 group-hover:translate-x-1 transition-transform flex-shrink-0 ml-2"
              />
            </Link>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {teams.map((team) => (
            <Link
              ...
              className="group flex flex-col items-center text-center p-3 rounded-2xl bg-[#070F1E] hover:bg-emerald-600/20 border border-blue-500/15 hover:border-emerald-400/40 transition-all"
            >
              <div className="h-10 w-10 rounded-xl bg-slate-900 border border-white/10 p-1 mb-2 flex items-center justify-center font-black text-emerald-400 text-sm group-hover:scale-105 transition-transform overflow-hidden">
                {team.logo ? (
                  <img
                    src={team.logo}
                    alt={team.name}
              ...
              <span className="text-xs font-bold text-slate-200 group-hover:text-emerald-300 truncate max-w-full">
                {team.name}
              </span>
      ...
      {miniStandings.length > 0 && (
        <div className="rounded-3xl border border-blue-500/20 bg-[#0A162B]/90 p-5 space-y-4 shadow-xl backdrop-blur-md">
          ...
          <div className="overflow-hidden rounded-2xl border border-white/5 bg-[#060D18]/80">
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-b border-white/10 text-[10px] font-black uppercase text-slate-400">
                  ...
                  <th className="py-2 px-2 text-center text-emerald-400">W</th>
                  <th className="py-2 px-2 text-center text-amber-400">PTS</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 font-mono">
                {miniStandings.map((s, idx) => (
                  <tr key={s.team_key || idx} className="hover:bg-white/5">
                    ...
                    <td className="py-2 px-2 text-center text-emerald-400 font-bold">{s.standing_W || '0'}</td>
                    <td className="py-2 px-2 text-center font-black text-amber-400">{s.standing_Pts || '0'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
