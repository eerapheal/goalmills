...
import { FiPlay, FiMail, FiArrowRight, FiCheck } from 'react-icons/fi';

function mapArticle(item: any, idx: number) {
  ...
  return {
    id: item._id || item.id || `art-${idx}`,
    slug: item.slug || (item.title ? slugify(item.title) : `art-${idx}`),
    title: item.title,
...
export function SportsPulseNewsSection({ initialArticles }: { initialArticles?: any[] } = {}) {
  ...
  const [articles, setArticles] = useState(() => {
    if (Array.isArray(initialArticles) && initialArticles.length > 0) {
      return initialArticles.slice(0, 8).map(mapArticle);
    }
    return [];
  });
  ...
  React.useEffect(() => {
    ...
    async function fetchLivePulseContent() {
      // Only fetch if no server-side articles were passed in
      if (Array.isArray(initialArticles) && initialArticles.length > 0) {
        // Fetch video only
        try {
          const videoRes = await fetch('/api/videos?limit=1').catch(() => null);
          if (videoRes && videoRes.ok && isMounted) {
      ...
      try {
        ...
        // silent fail — no mock data shown
      } finally {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
              {isLoadingArticles
                ? Array.from({ length: 6 }).map((_, i) => (
                    <div
                      key={i}
                      className="rounded-3xl border border-blue-500/10 bg-[#091529]/60 min-h-[260px] animate-pulse"
                    />
                  ))
