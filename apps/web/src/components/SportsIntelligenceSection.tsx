...
export function SportsIntelligenceSection() {
    ...
    const [voteCounts, setVoteCounts] = useState<{ [key: string]: { home: number; draw: number; away: number } }>({
        'match-1': { home: 54, draw: 18, away: 28 },
    });
    const [activeFeatureTab, setActiveFeatureTab] = useState<'analytics' | 'alerts' | 'h2h' | 'odds'>('analytics');
    ...
    const featureTabs = [
        {
            ...
            description: 'Advanced Monte Carlo match simulation models calibrated with historical form, expected goals (xG), team fitness, and tactical formations.',
            points: ['Live probability meters updated each minute', 'Expected Goals (xG) & Shot Momentum', 'Dangerous attacks & possession heatmaps'],
            badge: 'AI Powered',
        ...
        },
        {
            ...
            description: 'Receive instantaneous push updates for goals, red cards, cricket wickets, bowling spells, and NBA fourth-quarter clutch finishes.',
            points: ['Configurable favorite team notifications', 'Sound alerts for penalty and VAR checks', 'Cross-device instant web & mobile sync'],
            badge: 'Instant',
        ...
        },
        {
            ...
            description: 'Detailed past meetings, referee bias statistics, home vs away scoring records, and team form trends across the last 10 seasons.',
            points: ['Form guide streak indicators (W-D-L)', 'Clean sheet & BTTS (Both Teams To Score) %', 'Player disciplinary records & card histories'],
            badge: 'Pro Stats',
        ...
        },
        {
            ...
            description: 'Track real-time market movements, opening vs current betting lines, over/under spreads, and key match insights from global sportsbooks.',
            points: ['Real-time 1X2, Asian Handicap & Totals', 'Odds movement trend charts', 'Value indicator flags for smart insights'],
            badge: 'Market Radar',
    ...
    const topLeagues = [
        { name: 'Premier League', flag: '🏴󠁧󠁢󠁥󠁮󠁧󠁿', matches: '10 Fixtures', color: 'hover:border-purple-500/50' },
        { name: 'UEFA Champions League', flag: '⭐', matches: 'Elite Stage', color: 'hover:border-blue-500/50' },
        { name: 'La Liga', flag: '🇪🇸', matches: '10 Fixtures', color: 'hover:border-amber-500/50' },
        { name: 'NBA Basketball', flag: '🏀', matches: 'Live Tonight', color: 'hover:border-orange-500/50' },
        { name: 'IPL Cricket', flag: '🏏', matches: 'Matchday', color: 'hover:border-cyan-500/50' },
        { name: 'ATP & WTA Tour', flag: '🎾', matches: 'Grand Slam', color: 'hover:border-emerald-500/50' },
    ];
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center max-w-3xl mx-auto mb-14 md:mb-20">
                    ...
                    <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-white tracking-tight leading-tight">
                        Engineered for True Fans & <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-300 to-amber-300">Sports Analysts</span>
                    </h2>
                    <p className="mt-4 text-base md:text-lg text-slate-300 leading-relaxed">
                        GoalMills delivers real-time match telemetry, tactical player performance data, and intelligent win predictors across 1,200+ tournaments worldwide.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-16">
                    {statsMetrics.map((stat, idx) => (
                        ...
                        >
                            <div className={`absolute inset-0 bg-gradient-to-br ${stat.glow} rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none`} />
                    ...
                    <div className="lg:col-span-7 bg-slate-900/40 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 md:p-8 shadow-2xl">
                        ...
                        <div className="pt-6 animate-fade-in">
                            <div className="flex items-center justify-between gap-4 mb-3">
                                ...
                                <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${currentTabDetails.badgeColor}`}>
                    ...
                    <div className="lg:col-span-5 bg-gradient-to-br from-slate-900/90 via-slate-900/60 to-slate-950/90 backdrop-blur-xl border border-blue-500/20 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs font-black uppercase tracking-wider text-red-400">Match Pulse of the Day</span>
                            </div>
                        ...
                        <div className="w-full bg-slate-800 rounded-full h-3 flex overflow-hidden p-0.5 gap-0.5 mb-3">
                            <div style={{ width: `${homePct}%` }} className="bg-blue-500 h-full rounded-l-full transition-all duration-500" title={`Real Madrid: ${homePct}%`} />
                            <div style={{ width: `${drawPct}%` }} className="bg-amber-500 h-full transition-all duration-500" title={`Draw: ${drawPct}%`} />
                            <div style={{ width: `${awayPct}%` }} className="bg-cyan-400 h-full rounded-r-full transition-all duration-500" title={`Man City: ${awayPct}%`} />
                        </div>

                        <p className="text-[11px] text-slate-400 text-center">
                            {votedMatch ? '✓ Your prediction is recorded!' : 'Tap an outcome to register your fan prediction'}
                        </p>
                    </div>
                </div>
                ...
                <div className="border-t border-slate-800/80 pt-10">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
                        ...
                        <span className="text-xs text-slate-400">Instant access to fixtures, standings & top scorers</span>
                    </div>
