    ...
    try {
      ...
      if (rawFixtures.length > 0) {
        ...
        const seasons = Array.from(
          new Set(rawFixtures.map((f) => f.league_season).filter((s) => s))
        );
        if (seasons.length > 0) {
      ...
      <View>
        {standings.map((group, groupIndex) => (
          <View key={`group-${groupIndex}`} style={styles.standingsGroup}>
            {standings.length > 1 && <Text style={styles.groupTitle}>{group.name}</Text>}
              ...
              {group.teams.map((standing, index) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[styles.standingsRow, index < 4 && styles.championsLeagueRow]}
                    onPress={() => {
                  ...
                  >
                    <Text style={[styles.standingsText, styles.posCol]}>
                    ...
                    <Text style={[styles.standingsText, styles.statCol]}>
                      {standing.standing_P}
                    </Text>
                    <Text style={[styles.standingsText, styles.statCol]}>
                      {standing.standing_W}
