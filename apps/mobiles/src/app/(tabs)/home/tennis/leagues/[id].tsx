      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}>
          ...
          <Text style={styles.subtitle}>
            {league.country_name} • {league.league_surface}
          </Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {fixtures.filter((m) => m.event_live === '1' || m.event_status !== 'Finished').length >
          0 ? (
            fixtures
              .filter((m) => m.event_live === '1' || m.event_status !== 'Finished')
              .map((match) => <TennisMatchCard key={match.event_key} match={match} />)
          ) : (
        ...
        <View style={styles.section}>
          ...
          {fixtures.filter((m) => m.event_status === 'Finished' && m.event_live !== '1').length >
          0 ? (
            fixtures
              .filter((m) => m.event_status === 'Finished' && m.event_live !== '1')
              .map((match) => <TennisMatchCard key={match.event_key} match={match} />)
          ) : (
