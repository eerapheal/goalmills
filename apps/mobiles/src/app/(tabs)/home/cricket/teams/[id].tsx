    ...
    const loadTeamData = async () => {
      ...
      try {
        ...
        const matchFixture = allMatches.find(
          (m: any) =>
            String(m.home_team_key) === String(id) || String(m.away_team_key) === String(id)
        );
        ...
        <View style={styles.heroCard}>
          ...
          <View style={styles.badge}>
            <Text style={styles.badgeText}>
              {(team as any).country_name || 'OFFICIAL CRICKET SQUAD'}
            </Text>
          </View>
          <Text style={styles.statsOverview}>
            {players.length} Squad Athletes • {schedules.length} Upcoming • {results.length}{' '}
            Completed
          </Text>
        </View>
        ...
        {(activeTab === 'schedule' || activeTab === 'results') && (
          <View style={styles.sectionContainer}>
            ...
            {currentMatches.length > 0 ? (
              currentMatches.map((m) => <CricketMatchCard key={m.event_key} match={m} />)
            ) : (
              <View style={styles.emptyState}>
