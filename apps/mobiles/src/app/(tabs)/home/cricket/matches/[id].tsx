...
export default function CricketMatchDetailsScreen() {
    ...
    return (
      <View style={styles.errorContainer}>
        ...
        <Text style={styles.errorSubtext}>
          The match data could not be retrieved from the worldwide feed.
        </Text>
      ...
      <View style={{ gap: 12 }}>
        {Object.entries(match.scorecard).map(([innings, players]) => {
          ...
          const batters = players.filter(
            (p) => p.type === 'Batsman' || (p.R !== undefined && p.O === null)
          );
                ...
                <>
                  ...
                  {batters.map((batter, idx) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        router.push(
                          `/home/cricket/players/${(batter as any).player_id || (batter as any).player_key || encodeURIComponent(batter.player)}`
                        )
                      }
                    >
                      <View style={{ flex: 3 }}>
                      ...
                      <Text
                        style={[
                          styles.tableText,
                          { flex: 1, textAlign: 'right', fontWeight: 'bold', color: '#fbbf24' },
                        ]}
                      >
                        {batter.R}
                      </Text>
                      ...
                      <Text
                        style={[
                          styles.tableText,
                          { flex: 1, textAlign: 'right', color: '#60a5fa' },
                        ]}
                      >
                        {batter.SR}
                      </Text>
                ...
                <>
                  ...
                  {bowlers.map((bowler, idx) => (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        router.push(
                          `/home/cricket/players/${(bowler as any).player_id || (bowler as any).player_key || encodeURIComponent(bowler.player)}`
                        )
                      }
                    >
                      <View style={{ flex: 3 }}>
                      ...
                      <Text
                        style={[
                          styles.tableText,
                          { flex: 1, textAlign: 'right', fontWeight: 'bold', color: '#34d399' },
                        ]}
                      >
                        {bowler.W}
                      </Text>
      ...
      return (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>
            Live ball-by-ball commentary feed will initiate shortly.
          </Text>
        </View>
      );
    }
        ...
        {Object.entries(match.comments).map(([innings, comments]) => (
          <View key={innings} style={styles.card}>
            <Text style={styles.sectionTitle}>
              {innings.replace('_', ' ').toUpperCase()} COMMENTARY
            </Text>
            {comments.map((c, i) => (
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {c.runs !== '0' && (
                    <Text
                      style={[
                        styles.commentRuns,
                        (c.runs === '4' || c.runs === '6' || c.runs === 'W') && styles.boundaryRuns,
                      ]}
                    >
  ...
  const renderLineups = () => {
    if (
      !match.lineups ||
      (!match.lineups.home_team?.starting_lineups?.length &&
        !match.lineups.away_team?.starting_lineups?.length)
    ) {
      return (
        <View style={styles.emptyState}>
    ...
    return (
      <View style={{ gap: 12 }}>
        <View style={styles.card}>
          <TouchableOpacity
            onPress={() =>
              match.home_team_key && router.push(`/home/cricket/teams/${match.home_team_key}`)
            }
          >
          ...
          {match.lineups.home_team.starting_lineups.map((p, i) => (
            <TouchableOpacity
              ...
              onPress={() =>
                router.push(
                  `/home/cricket/players/${(p as any).player_id || (p as any).player_key || encodeURIComponent(p.player)}`
                )
              }
            >
              ...
              {(p as any).is_captain && (
                <Text style={{ color: '#fbbf24', fontSize: 10, fontWeight: '900', marginRight: 6 }}>
                  (C)
                </Text>
              )}
              {(p as any).is_keeper && (
        ...
        <View style={styles.card}>
          <TouchableOpacity
            onPress={() =>
              match.away_team_key && router.push(`/home/cricket/teams/${match.away_team_key}`)
            }
          >
    ...
    return (
      <View style={{ gap: 12 }}>
        {Object.entries(match.wickets).map(([innings, wickets]) => (
          <View key={innings} style={styles.card}>
            <Text style={styles.sectionTitle}>
              FALL OF WICKETS - {innings.replace('_', ' ').toUpperCase()}
            </Text>
            {wickets.map((w, idx) => (
              <View key={idx} style={styles.tableRow}>
                ...
                >
                  <Text style={{ color: COLORS.secondary, fontSize: 10, fontWeight: '900' }}>
                    {idx + 1}
                  </Text>
                </View>
                <View style={{ flex: 1 }}>
      ...
      <View style={{ gap: 10 }}>
        {h2h.H2H.map((item, idx) => (
          <View key={idx} style={styles.card}>
            ...
            {item.event_status_info && <Text style={styles.h2hInfo}>{item.event_status_info}</Text>}
          </View>
        ))}
      </View>
    );
          ...
          <View style={{ marginTop: 16 }}>
            ...
            {Object.entries(odds).map(([market, outcomes]) => (
              <View key={market} style={{ marginTop: 8 }}>
                <Text
                  style={{ color: '#fbbf24', fontSize: 10, fontWeight: '800', marginBottom: 4 }}
                >
                  {market}
                </Text>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  {Object.entries(outcomes).map(([outcome, bookmakers]) => (
                    <View key={outcome} style={styles.oddsBox}>
                      ...
                      <Text style={styles.oddsVal}>{Object.values(bookmakers)[0] || '1.90'}</Text>
                    </View>
                  ))}
                </View>
              </View>
  ...
  const renderInfo = () => (
    <View style={styles.card}>
      ...
      <View style={styles.infoRow}>
        ...
        <Text style={styles.infoValue}>
          {match.event_date_start} • {match.event_time}
        </Text>
      </View>
      <View style={styles.infoRow}>
        <Text style={styles.infoLabel}>Venue</Text>
        <Text style={styles.infoValue}>
          {match.event_stadium || 'Global International Stadium'}
        </Text>
      </View>
      {match.event_toss && (
        <View style={styles.infoRow}>
