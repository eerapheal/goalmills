...
import { View, Text, StyleSheet, FlatList, ScrollView, ActivityIndicator, TouchableOpacity, Image } from 'react-native';
            ...
            <View style={styles.header}>
                <View style={styles.headerImage}>
                    ...
                    <View style={styles.headerContentOverlay}>
                        ...
                        <Text style={styles.subtitle}>{series.country_name || 'Tournament Hub'} • {matches.length} Matches</Text>
                    </View>
                </View>
            </View>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            <Text style={styles.emptyText}>No scheduled fixtures or recent results available.</Text>
                        </View>
                    }
                />
            )}
                            ...
                            {standings.map((row, idx) => (
                                <View key={idx} style={styles.tableRow}>
                                    <View style={{ flex: 3, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                        ...
                                        <Text style={styles.teamNameText} numberOfLines={1}>{row.team_name || row.team_short_name}</Text>
                                    </View>
                                    <Text style={[styles.tableCell, { flex: 1, textAlign: 'center' }]}>{row.standing_P}</Text>
                                    <Text style={[styles.tableCell, { flex: 1, textAlign: 'center', color: '#10b981' }]}>{row.standing_W}</Text>
                                    <Text style={[styles.tableCell, { flex: 1, textAlign: 'center', color: '#f43f5e' }]}>{row.standing_L}</Text>
                                    <Text style={[styles.tableCell, { flex: 1.2, textAlign: 'center', fontWeight: '900', color: '#fbbf24' }]}>{row.standing_PTS}</Text>
                                    <Text style={[styles.tableCell, { flex: 1.5, textAlign: 'right', color: 'rgba(255,255,255,0.6)' }]}>{row.standing_NRR}</Text>
                                </View>
                            ))}
                        </View>
                    ) : (
                        <View style={styles.emptyState}>
                            <Text style={styles.emptyText}>Points table standings are currently being compiled.</Text>
                        </View>
                    )}
                </ScrollView>
            )}
                        ...
                        squads.map((sq, idx) => (
                            <View key={idx} style={styles.squadCard}>
                                <View style={styles.squadAvatar}>
                                    <Text style={styles.squadInitial}>{sq.squad_name ? sq.squad_name.charAt(0) : 'S'}</Text>
                                </View>
                                <View style={{ flex: 1 }}>
                        ...
                        venues.map((v, idx) => (
                            <View key={idx} style={styles.venueCard}>
                                ...
                                <View style={{ flex: 1, marginLeft: 12 }}>
                                    ...
                                    <Text style={styles.venueSubtitle}>{v.city}{v.country ? `, ${v.country}` : ''}</Text>
                                    {v.capacity && (
                                        <Text style={styles.venueCapacity}>Capacity: {v.capacity}</Text>
                                    )}
                                </View>
                            </View>
                        ))
                    ...
                    {news.length > 0 ? (
                        news.map((item, idx) => (
                            <View key={idx} style={styles.newsCard}>
                                ...
                                <Text style={styles.newsTitle}>{item.headline || item.title || 'Cricket News'}</Text>
                                <Text style={styles.newsIntro} numberOfLines={2}>{item.intro || item.description || ''}</Text>
                            </View>
                        ))
                    ) : (
                        <View style={styles.emptyState}>
