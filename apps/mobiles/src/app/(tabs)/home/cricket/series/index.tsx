...
export default function CricketSeriesListScreen() {
  ...
  const renderItem = ({ item }: { item: CricketLeague }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => router.push(`/home/cricket/series/${item.league_key}`)}
    >
      {/* Start using league_logo if available, else placeholder */}
      <View
        style={[
          styles.cardImage,
          { justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.secondary },
        ]}
      >
        {item.league_logo ? (
          <Image source={{ uri: item.league_logo }} style={{ width: '100%', height: '100%' }} />
        ) : (
          <Text style={{ fontSize: 32, fontWeight: 'bold', color: COLORS.background }}>
            {item.league_name.charAt(0)}
          </Text>
        )}
      </View>
      <View style={styles.cardContent}>
        ...
        <View style={styles.details}>
          <View style={styles.row}>
            ...
            <Text style={styles.detailText}>{item.league_year}</Text>
          </View>
          {item.country_name && (
            <View style={styles.row}>
