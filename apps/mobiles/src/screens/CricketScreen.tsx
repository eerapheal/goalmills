...
export function CricketScreen() {
    ...
    const loadData = async () => {
        try {
            ...
            const uniqueTeams = Array.from(
                new Map(allTeamsCombined.filter(t => t && t.team_key).map(t => [String(t.team_key), t])).values()
            );
            ...
            setRankings({
                ...
                'T20 WC': t20WcRank.result?.total || (Array.isArray(t20WcRank.result) ? t20WcRank.result : []),
    ...
    const filterMatches = (matches: CricketEvent[]) => {
        return matches.filter(m => {
            ...
            if (formatFilter === 'international') {
                return type.includes('test') || type.includes('odi') || type.includes('t20i') || league.includes('tour') || league.includes('icc') || league.includes('cup') || league.includes('trophy');
            }
            if (formatFilter === 'franchise') {
                return league.includes('ipl') || league.includes('premier') || league.includes('bbl') || league.includes('psl') || league.includes('super league') || league.includes('hundred') || league.includes('cpl');
            }
            if (formatFilter === 'women') {
                return league.includes('women') || league.includes('wpl') || league.includes('wbbl');
            }
            if (formatFilter === 'domestic') {
                return league.includes('trophy') || league.includes('shield') || league.includes('ranji') || league.includes('county');
            }
    ...
    const filteredLive = useMemo(() => filterMatches(liveMatches), [liveMatches, formatFilter, searchQuery]);
    const filteredUpcoming = useMemo(() => filterMatches(upcomingMatches), [upcomingMatches, formatFilter, searchQuery]);
    const filteredRecent = useMemo(() => filterMatches(recentMatches), [recentMatches, formatFilter, searchQuery]);
        ...
        <View style={styles.headerSection}>
            <View style={styles.heroContent}>
                ...
                <Text style={styles.heroSubtitle}>
                    Real-time match analytics, ball-by-ball scorecards, athlete profiles, and tournament standings.
                </Text>
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll} contentContainerStyle={styles.filterContainer}>
                {formatFilters.map(filter => (
                    ...
                    >
                        <Text style={[styles.filterChipText, formatFilter === filter.id && styles.filterChipTextActive]}>
                            {filter.label}
                        </Text>
                    </TouchableOpacity>
                ))}
                    ...
                    <View style={styles.tabContent}>
                        ...
                        {filteredLive.length > 0 ? (
                            ...
                            <View style={styles.emptyState}>
                                ...
                                <Text style={styles.emptySubtext}>No professional matches active in this format right now.</Text>
                            </View>
                        )}
                    </View>
                );
                    ...
                    <View style={styles.tabContent}>
                        ...
                        {filteredUpcoming.length > 0 ? (
                            ...
                            <View style={styles.emptyState}>
                                ...
                                <Text style={styles.emptySubtext}>No upcoming schedule for the selected filter.</Text>
                            </View>
                        )}
                    </View>
                );
                    ...
                    <View style={styles.tabContent}>
                        ...
                        {filteredRecent.length > 0 ? (
                            ...
                            <View style={styles.emptyState}>
                                ...
                                <Text style={styles.emptySubtext}>No completed fixtures recorded for this filter.</Text>
                            </View>
                        )}
                    </View>
                );
                            ...
                            {filteredSeries.map((series) => (
                                ...
                                >
                                    <View style={styles.seriesLogoContainer}>
                                        <Text style={styles.seriesInitial}>{(series.league_name || 'S').charAt(0)}</Text>
                                    </View>
                                ...
                                >
                                    <View style={styles.playerAvatar}>
                                        {player.player_image ? (
                                            ...
                                            <Text style={styles.playerAvatarText}>{(player.player_name || 'P').charAt(0)}</Text>
                                        )}
                                    </View>
                                    <View style={{ flex: 1, marginLeft: 12 }}>
                                        <Text style={styles.playerName}>{player.player_name}</Text>
                                        <Text style={styles.playerRole}>{player.player_type || player.player_role || 'Athlete'} • {player.player_country || 'International'}</Text>
                                    </View>
                                ...
                                {standings.length > 0 ? (
                                    <View style={styles.rankingList}>
                                        <View style={styles.rankHeaderRow}>
                                            ...
                                            <Text style={[styles.rankHeadCol, { width: 28, textAlign: 'center' }]}>P</Text>
                                            <Text style={[styles.rankHeadCol, { width: 28, textAlign: 'center' }]}>W</Text>
                                            <Text style={[styles.rankHeadCol, { width: 28, textAlign: 'center' }]}>L</Text>
                                            <Text style={[styles.rankHeadCol, { width: 36, textAlign: 'right' }]}>PTS</Text>
                                        </View>
                                        {standings.map((row, idx) => (
                                            <Pressable
                                                key={idx}
                                                style={styles.rankingRow}
                                                onPress={() => row.team_key && router.push(`/home/cricket/teams/${row.team_key}`)}
                                            >
                                                <Text style={styles.rankNumber}>{row.standing_place || idx + 1}</Text>
                                                <Text style={styles.rankName} numberOfLines={1}>{row.standing_team}</Text>
                                    ...
                                    <View style={styles.rankingEmpty}>
                                        <Text style={styles.rankingEmptyText}>Awaiting season points confirmation.</Text>
                                    </View>
                                )}
                            </View>
                        ))}
