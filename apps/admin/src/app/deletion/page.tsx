      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Audit-protected soft-deletion system. Recover accidentally removed articles, media, and
            campaigns or permanently purge.
          </p>
        </div>
      ...
      <div className="flex items-start gap-4 p-4 rounded-2xl bg-slate-900/90 border border-rose-500/30 shadow-lg">
        ...
        <div>
          ...
          <p className="text-xs text-slate-300 mt-0.5 leading-relaxed">
            GoalMills enforces two-stage deletion. Items placed in the Trash Bin retain metadata and
            can be safely restored to drafts by Editors. Permanent purging requires
            Manager/Super-Admin authorization and is permanently logged in the audit trail.
          </p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-white/5">
                {deletedItems.map((item) => (
                  <tr key={item.id} className="hover:bg-white/5 transition">
                    ...
                    <td className="py-3 px-3 text-xs text-slate-400">{item.deletedBy}</td>
