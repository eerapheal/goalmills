...
export default function DailyReportsAdminPage() {
  ...
  const [reviewStatus, setReviewStatus] = useState<
    'approved' | 'reviewed' | 'revision' | 'retraining'
  >('approved');
      ...
      <div className="glass-card p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-white/10 shadow-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-text-muted mt-0.5">
            Review and grade trainee assignments using the official 10-category 100-point GoalMills
            Sports Media Academy rubric
          </p>
        </div>
              ...
              >
                ...
                {(rep.articleUrl ||
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {rep.facebookUrl && (
                      <a
                        href={rep.facebookUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="px-2.5 py-1 rounded-lg bg-blue-600/10 text-blue-300 border border-blue-600/20 text-[11px] font-bold hover:bg-blue-600/20 flex items-center gap-1"
                      >
                  ...
                  <div className="p-3 bg-slate-900/40 rounded-xl border border-white/5 text-xs space-y-1">
                    ...
                    {rep.source1 && (
                      <p className="text-slate-300">
                        Source 1: <span className="text-slate-200">{rep.source1}</span>
                      </p>
                    )}
                    {rep.source2 && (
                      <p className="text-slate-300">
                        Source 2: <span className="text-slate-200">{rep.source2}</span>
                      </p>
                    )}
                  </div>
                )}
                ...
                {rep.scorecard && rep.totalScore && (
                  <div className="p-3 sm:p-4 bg-slate-900/60 rounded-xl border border-white/5 space-y-2">
                    <p className="text-xs font-bold text-amber-400 flex items-center gap-1">
                      <FiBarChart2 size={12} /> 10-Category Scorecard ({rep.totalScore}/100 —{' '}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                      {DAILY_SCORECARD_RUBRICS.map((rubric) => {
                        ...
                        return (
                          <div
                            key={rubric.key}
                            className="text-center p-1.5 rounded-lg bg-slate-950 border border-white/5"
                          >
                            ...
                            <span className="text-sm font-black text-amber-400">
                              {val}
                              <span className="text-[10px] text-slate-500">/{rubric.maxScore}</span>
                            </span>
                          </div>
                        );
                      })}
            ...
            >
              ...
              <div className="flex items-center justify-between bg-slate-950 p-4 rounded-2xl border border-white/10">
                ...
                <div className="text-right">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${getRatingColor(totalCurrentScore)} bg-white/5 border border-white/10`}
                  >
              ...
              <div className="space-y-3">
                ...
                {DAILY_SCORECARD_RUBRICS.map((rubric) => {
                  ...
                  return (
                    <div
                      key={rubric.key}
                      className="bg-slate-950/60 p-3 rounded-xl border border-white/5"
                    >
              ...
              <div className="pt-3 border-t border-white/10 flex items-center justify-end gap-3">
                ...
                >
                  {submittingReview
                    ? 'Grading & Notifying...'
                    : `Submit Score (${totalCurrentScore}/100)`}
                </button>
              </div>
            </form>
          </div>
