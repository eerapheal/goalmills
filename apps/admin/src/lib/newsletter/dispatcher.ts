...
export async function dispatchNewsletter(params: DispatchCampaignParams): Promise<DispatchResult> {
  ...
  // If Go microservice dispatch failed, trigger Direct SMTP fallback
  ...
  if (!dispatchedViaGo) {
    ...
    if (directFallbackSuccessCount > 0) {
      dispatchedViaGo = true; // successfully delivered via fallback
    }
  }

  // If both Go mailer and direct SMTP fallback failed
  if (!dispatchedViaGo) {
  ...
  return {
    ...
    eligibleCount: eligibleSubscribers.length,
    suppressedCount: report.suppressedCount,
    message: `Dispatched to ${eligibleSubscribers.length} deliverable subscribers via Go Domain Router (${report.suppressedCount} suppressed)`,
    preflightReport: report,
  };
}
